...
const ClassRoutine = () => {
  ...
  const handleSelectAllChange = () => {
    if (selectedUsers.length === data.length) {
      ...
      setSelectedUsers(data.map((room) => room.id));
    }
  };
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      let url;
      let responseData = await API.getClassRoutine(reqPage, rowsPerPage);
      if (searchTerm) {
      ...
      if (responseData && responseData.data) {
        const updatedClassRoutine = responseData.data.map((room) => {
          const capitalizedFullName =
            room.room_name.charAt(0).toUpperCase() + room.room_name.slice(1);
          return { ...room, room_name: capitalizedFullName };
        });

        console.log("Filtered ClassRoutine (excluding super_admin):", updatedClassRoutine);

        setData(updatedClassRoutine);
  ...
  const columns = [
    ...
    {
      name: "Room_number",
      selector: (row) => <span>{row.room_number}</span>,
      width: "10%",
    },
    {
      name: "Name",
      selector: (row) => <span> {row.room_name}</span>,
      width: "20%",
    },
    {
      name: "Floor",
      selector: (row) => <span> {row.floor}</span>,
      width: "8%",
    },
    {
      name: "Capacity",
      selector: (row) => <span> {row.capacity}</span>,
      width: "8%",
    },
  ...
  const handleEdit = (roomid) => {
    ...
    setSelectedUser(roomid); // Store the selected user for editing
  ...
  const handleDeleteUser = async () => {
    ...
    if (validUsers.length === 0) {
      alert("Invalid room IDs provided.");
      return;
    }
    setIsModalOpen(true);

    try {
      const url = "/ClassRoutine/delete"; // Use the correct delete API endpoint
      const payload = {
  ...
  return (
    <div className="relative p-4">
      <div className="list-user-title ">
        <h2 className="text-xl font-bold sub-title">List of ClassRoutine</h2>
      </div>
        ...
        <div className="combine-export-section">
          <div className="input-search-bar flex ">
            <input
              ...
              placeholder="Search ClassRoutine"
