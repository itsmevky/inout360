...
const Departments = () => {
  ...
  const fetchdepartment = async () => {
    ...
    try {
      ...
      let url;
      let responseData = await API.getDepartments(reqPage, rowsPerPage);
      ...
      if (responseData && responseData.data) {
        const updatedDepartments = responseData.data.map((item) => {
          const capitalizedName =
            item.name.charAt(0).toUpperCase() + item.name.slice(1);

          const updatedSubDepartments = (item.subDepartments || []).map(
            (sub) => {
              const capitalizedSubName =
                sub.name.charAt(0).toUpperCase() + sub.name.slice(1);
              return { ...sub, name: capitalizedSubName };
            }
          );
        ...
        console.log(
          "Filtered department (excluding super_admin):",
          updatedDepartments
        );
  ...
  const columns = [
    ...
    {
      name: "Code",
      selector: (row) => <span>{row.code}</span>,
      width: "20%",
    },
    {
      name: "Name",
    ...
    },
    {
      name: "Head",
      selector: (row) => {
        const head = row?.head?.toLowerCase();
    ...
    },
    {
      ...
      selector: (row) => {
        const departmenttype = row?.departmentType?.toLowerCase();
        return departmenttype
          ? departmenttype.charAt(0).toUpperCase() + departmenttype.slice(1)
          : "";
      },
      width: "8%",
    },
  ...
  const handleApplyClick = async () => {
    ...
    try {
      ...
      if (response.status === true) {
        setSelectedStatus("");
        setSelectedUsers("");
        fetchdepartment(); // Refresh user list
  ...
  const handleSearchChange = (e) => {
    ...
    fetchdepartment(); // Trigger fetch with the updated search term
  };
  ...
  return (
    <div className="relative p-4">
      <div className="list-user-title ">
        <h2 className="text-xl font-bold sub-title">List of Department</h2>
      </div>
