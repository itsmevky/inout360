...
const Courses = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      let url;
      let responseData = await API.getDepartmentList(reqPage, rowsPerPage);
      if (searchTerm) {
      ...
      if (responseData && responseData.data) {
        const updatedUsers = responseData.data;
  ...
  const columns = [
...

    {
      ...
      selector: (row) => <span>{row.departmentType}</span>,
      width: "10%",
    },
    {
      name: "Head",
      selector: (row) => <span>{row.head}</span>,
      width: "10%",
    },
  ...
  return (
    <div className="relative p-4">
      <div className="list-user-title ">
        <h2 className="text-xl font-bold sub-title">Department</h2>
      </div>
