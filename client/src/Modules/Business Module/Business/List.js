...
import AddUserForm from "./Addbusiness.js";
...
const GetUsers = () => {
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      if (res.status) {
        ...
        fetchBusiness(); // Refresh user list
      } else {
  ...
  const fetchBusiness = async () => {
    ...
    try {
      ...
      // 🔍 If there's a search term, pass it
      if (searchTerm && searchTerm.trim() !== "") {
        params.search = searchTerm.trim();
      }

      // 🧠 Module name based on your dynamic router (e.g., "business_types")
      const module = "businesses";

      // 📡 Call the generic GET function for this module
      const responseData = await API.getAll(module, params); // ← Ensure this matches your API utility
      ...
      if (responseData.data) {
        const updatedBusiness = responseData.data.map((item) => {
          const capitalizedType = item.type
            ? item.type.charAt(0).toUpperCase() + item.type.slice(1)
            : "Unknown";
          return { ...item, type: capitalizedType };
        });

        console.log("Fetched Business Data:", updatedBusiness);
              ...
              onClick={async () => {
                ...
                try {
                  const results = await Promise.allSettled(
                    selectedUsers.map((id) => API.remove("businesses", id))
                  );
  ...
  const columns = [
...

    {
      name: "Business Type",
      selector: (row) => (
        <span>{capitalizeFirstLetter(row.business_type)}</span>
      ),
      width: "10%",
    },
    ...
    {
      name: "Domain",
      selector: (row) => <span>{row.domain}</span>,
      width: "10%",
    },
    {
      name: "Status",
  ...
  const handleApplyClick = async (module = "businesses") => {
    if (typeof module !== "string") {
      toast.error("Invalid module name.");
      console.error("❌ Module must be a string, received:", module);
      return;
    }
    ...
    try {
      ...
      if (res.status) {
        ...
        setSelectedUsers([]);
        setSelectedStatus("");
        fetchBusiness(); // or fetchData() for dynamic use
      } else {
    ...
    try {
      ...
      await Promise.all(
        validUsers.map(async (id) => {
          try {
            await API.singlebusinessDelete(id);
          } catch (err) {
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const response = await API.remove("businesses", userId); // ✅ uses api.js
  ...
  return (
    <div className="relative p-4">
      <div className="list-user-title ">
        <h2 className="text-xl font-bold sub-title">List of Businesses</h2>
      </div>
