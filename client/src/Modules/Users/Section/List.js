...
console.log("data", data);
  ...
  const fetchParents = async () => {
    ...
    try {
      ...
      const responseData = await API.section.getAll(params);    
  ...
  const handleApplyClick = async () => {
    ...
    try {
      ...
      if (response.status === true) {
        setSelectedStatus("");
        setSelectedUsers("");
        fetchParents(); // Refresh user list
  ...
  const handleSearchChange = (e) => {
    ...
    fetchParents(); // Trigger fetch with the updated search term
  };
  ...
  const handleStatusChange = async (studentIds, newStatus) => {
    try {
      const idsArray = Array.isArray(studentIds) ? studentIds : [studentIds];
      let result = await putData("/student/status", {
        students: idsArray,
        status: newStatus,
      });
  ...
  const handleDelete = async (studentIds) => {
    ...
    try {
      const payload = {
        students: Array.isArray(studentIds) ? studentIds : [studentIds],
      };
      const response = await deleteData("/student/delete", payload);
  ...
  const columns = [
    ...
    {
      name: "Email",
      selector: (row) => row.email.charAt(0).toUpperCase() + row.email.slice(1),
      width: "15%",
    },
    {
      name: "Status",
