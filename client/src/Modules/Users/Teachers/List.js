...
import AddUserForm from "./addteacher.js";
...
const GetTeachers = () => {
  ...
  const handleStatusChange = async (userId, newStatus, module = "teacher") => {
    try {
      ...
      if (res.status) {
        ...
        fetchteachers(); // Refresh user list
      } else {
  ...
  const handleSelectAllChange = () => {
    if (selectedteachers.length === data.length) {
      ...
      setSelectedteachers(data.map((user) => user.id));
    }
  };
  ...
  const fetchteachers = async () => {
    ...
    try {
      ...
      const responseData = await API.getAll("teacher", params);
  ...
  const columns = [
    ...
    },
    {
      name: "Department",
      selector: (row) => (
        <span>{row.professional_info?.[0]?.department || "N/A"}</span>
      ),
      width: "10%",
    },
    {
      name: "Designation",
      selector: (row) => (
        <span>{row.professional_info?.[0]?.designation || "N/A"}</span>
      ),
      width: "15%",
    },
    {
  ...
  const handleClick = () => {
    navigate("/dashboard/users/teachers/add");
  };
  ...
  const handleApplyClick = async (module = "teacher") => {
    ...
    try {
      ...
      if (res.status) {
        ...
        fetchteachers(); // Replace with fetchData() for other modules if needed
      } else {
  ...
  const updateteacherstatus = async (teachers, status) => {
    try {
      return await API.updateStatus({ teachers, status });
    } catch (error) {
      return error;
    }
  };
              ...
              onClick={async () => {
                ...
                try {
                  const results = await Promise.allSettled(
                    selectedUsers.map((id) => API.remove("teacher", id))
                  );
  ...
  const handleSearchChange = (e) => {
    ...
    fetchteachers(); // Trigger fetch with the updated search term
  };
  ...
  const handleDeleteUser = async () => {
    ...
    const validteachers = selectedteachers.filter((id) =>
    ...
    try {
      console.log("Deleting valid teachers:", validteachers);

      await Promise.all(
        validteachers.map(async (id) => {
          try {
      ...
      toast.error("Failed to delete some teachers. Try again.");
    }
  };
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const response = await API.remove("teacher", userId); // ✅ uses api.js
      ...
      <div className="button-crm">
        ...
        <div className="combine-export-section">
          ...
          <div className="crm-buttonsection">
            <a href="/dashboard/users/teachers/add">
              <button>
                {" "}
                <svg
                  fill="white"
