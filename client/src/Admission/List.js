...
const Get = () => {
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      if (res.status) {
        ...
        fetchAdmission(); // Refresh user list
      } else {
  ...
  const fetchAdmission = async () => {
    ...
    try {
      ...
      const responseData = await API.getAll("admission", params);

      if (!responseData || responseData.status === false) {
        throw new Error(
          responseData?.message || "Failed to fetch admission data"
        );
      }

      if (responseData.data) {
        const updatedAdmissions = responseData.data.map((admission) => {
          const capitalizedName =
            admission.fullname?.charAt(0).toUpperCase() +
            admission.fullname?.slice(1);
          return { ...admission, fullname: capitalizedName };
        });

        setData(updatedAdmissions);
  ...
  const columns = [
    ...
    // {
    //   name: "Profile Image",
    //   selector: (row) => <span>{row.profile_image}</span>,
    //   width: "20%",
    // },
    {
      name: "Roll No.",
      selector: (row) => (
        <span>{row.academic_info?.[0]?.roll_number || "-"}</span>
      ),
      width: "10%",
    },
    ...
    },
    {
      name: "Class",
      selector: (row) => {
        const classValue = row.academic_info?.[0]?.class_applied;
        return (
          <span>
            {classValue ? getOrdinalSuffix(parseInt(classValue)) : "-"}
          </span>
        );
      },
      width: "10%",
    ...
    {
      name: "Section",
      selector: (row) => <span>{row.academic_info?.[0]?.section || "-"}</span>,
      width: "10%",
    },
    {
      name: "Academic Year",
      selector: (row) => <span>{row.academic_year}</span>,
      width: "20%",
    },
    {
      name: "Status",
  ...
  const handleApplyClick = async (module = "admission") => {
    ...
    try {
      ...
      if (res.status) {
        ...
        fetchAdmission(); // Replace with fetchData() for other modules if needed
      } else {
  ...
  const handleSearchChange = (e) => {
    ...
    fetchAdmission(); // Trigger fetch with the updated search term
  };
    ...
    try {
      ...
      await Promise.all(
        validUsers.map(async (id) => {
          try {
            await API.multipleUserDelete(id);
          } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        fetchAdmission(); // Refresh the user list
      } else {
              ...
              onClick={async () => {
                ...
                try {
                  const results = await Promise.allSettled(
                    selectedUsers.map((id) => API.remove("admission", id))
                  );
  ...
  return (
    <div className="relative p-4">
      <div className="list-user-title ">
        <h2 className="text-xl font-bold sub-title">List of Admissions</h2>
      </div>
      <div className="button-crm">
        ...
        <div className="combine-export-section">
          ...
          <div className="crm-buttonsection">
            <a href="/dashboard/users/admission/add">
              <button>
                {" "}
                <svg
                  fill="white"
