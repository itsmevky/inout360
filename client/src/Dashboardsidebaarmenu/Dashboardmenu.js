...
const SuperAdminDashboardSidebar = ({ isCollapsed, setIsCollapsed }) => {
  const handleLogout = () => {
  ...
  const navItems = [
    ...
    },
    {
      ...
      links: [{ label: "Zones", path: "/dashboard/users/zones" }],
    },
  ];
...
const AdminDashboardSidebar = ({ isCollapsed, setIsCollapsed }) => {
  const handleLogout = () => {
    localStorage.removeItem("accesstoken");
    Cookies.remove("accesstoken");
    Cookies.remove("userdetail");
